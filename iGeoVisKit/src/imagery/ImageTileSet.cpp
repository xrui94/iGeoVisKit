...
char* ImageTileSet::get_tile_RGB(int x, int y, int band_R, int band_G, int band_B)
{
    ...
    /* Make room to put texture in (support multi-byte samples) */
    size = tex_size * tex_size * 3 * sample_size;
    ...
    if (sample_size == 1) {
        if (band_R && band_G && band_B) {
            ...
            while (pix < size) {
                if (band_R) { out_tile[pix]   = tile[piy+band_R-1]; band_R_min = std::min(band_R_min, (int)(unsigned char)out_tile[pix]); band_R_max = std::max(band_R_max, (int)(unsigned char)out_tile[pix]); } else { out_tile[pix]   = 0; }
                if (band_G) { out_tile[pix+1] = tile[piy+band_G-1]; band_G_min = std::min(band_G_min, (int)(unsigned char)out_tile[pix+1]); band_G_max = std::max(band_G_max, (int)(unsigned char)out_tile[pix+1]); } else { out_tile[pix+1] = 0; }
                if (band_B) { out_tile[pix+2] = tile[piy+band_B-1]; band_B_min = std::min(band_B_min, (int)(unsigned char)out_tile[pix+2]); band_B_max = std::max(band_B_max, (int)(unsigned char)out_tile[pix+2]); } else { out_tile[pix+2] = 0; }
                pix = pix + 3;
                piy = piy + num_bands;
            }
        }
    } else if (sample_size == 2) {
        // 16 位输出三通道，采用与 8 位相同的像素顺序遍历，确保行列与边界填充一致
        unsigned short* src16 = (unsigned short*)tile;
        unsigned short* out16 = (unsigned short*)out_tile;
        int pix16 = 0;         // 输出缓冲（以 16 位元素计）
        int piy16 = 0;         // 源缓冲（以像素为单位，每像素 num_bands 个 16 位）
        int totalElems = tex_size * tex_size * 3; // 输出总元素数（16 位）
        if (band_R && band_G && band_B) {
            int rIdx = band_R - 1;
            int gIdx = band_G - 1;
            int bIdx = band_B - 1;
            while (pix16 < totalElems) {
                unsigned short r = src16[piy16 + rIdx];
                unsigned short g = src16[piy16 + gIdx];
                unsigned short b = src16[piy16 + bIdx];
                out16[pix16] = r; out16[pix16+1] = g; out16[pix16+2] = b;
                band_R_min = std::min(band_R_min, (int)r); band_R_max = std::max(band_R_max, (int)r);
            ...
            while (pix16 < totalElems) {
                unsigned short r = band_R ? src16[piy16 + band_R - 1] : 0;
                unsigned short g = band_G ? src16[piy16 + band_G - 1] : 0;
                unsigned short b = band_B ? src16[piy16 + band_B - 1] : 0;
                out16[pix16] = r; out16[pix16+1] = g; out16[pix16+2] = b;
                band_R_min = std::min(band_R_min, (int)r); band_R_max = std::max(band_R_max, (int)r);
                band_G_min = std::min(band_G_min, (int)g); band_G_max = std::max(band_G_max, (int)g);
                band_B_min = std::min(band_B_min, (int)b); band_B_max = std::max(band_B_max, (int)b);
        ...
        // 其它字宽暂当 8 位处理，保证功能可用
        while (pix < size) {
            if (band_R) out_tile[pix]   = tile[piy+band_R-1]; else out_tile[pix]   = 0;
            if (band_G) out_tile[pix+1] = tile[piy+band_G-1]; else out_tile[pix+1] = 0;
            if (band_B) out_tile[pix+2] = tile[piy+band_B-1]; else out_tile[pix+2] = 0;
            pix = pix + 3;
            piy = piy + num_bands;
        }
    }
...
unsigned char* ImageTileSet::get_pixel_values(int x, int y)
{
    ...
    // 考虑 sample_size：计算字节偏移
    int pixel_start_bytes = ((x * num_bands) + (y * tex_size * num_bands)) * sample_size;
    if (sample_size == 1) {
        for (short tmp = 0; tmp < num_bands; tmp++) {
            return_values[tmp] = tile_data[pixel_start_bytes + tmp];
        }
    } else if (sample_size == 2) {
        unsigned short* p16 = (unsigned short*)tile_data;
        int base16 = (x * num_bands) + (y * tex_size * num_bands);
        for (short tmp = 0; tmp < num_bands; tmp++) {
