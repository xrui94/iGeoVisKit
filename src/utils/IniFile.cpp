...
bool iniFile::parse (string section, string key) {
    ...
    if (gotFileName) {
        std::ifstream filePtr (fileName.c_str());    // open the file for reading
...
void iniFile::update (string section, string key, string data) {
    ...
    if (gotFileName) {
        std::ifstream filePtrI (fileName.c_str());       // open the original file for reading
        std::ofstream filePtrO (".temp", std::ios::trunc);    // create temp file, erase contents if any
        ...
        // both files could not be opened together (but one may have been)
            ...
            if (!filePtrI.is_open() && filePtrO.is_open()) {
                ...
                std::ofstream filePtrO (fileName.c_str(), std::ios::app);
                if (filePtrO.is_open()) {
...
void iniFile::clearFileContents () {
    if (gotFileName) {                                     // make sure a filename was given
        std::ofstream filePtrO (fileName.c_str(), std::ios::trunc);  // open the file, erasing the contents
        if (filePtrO.is_open())
