...
ROI* ROISet::new_region (void) {
    ...
    std::string name = makeMessage("ROI-", i);
    while (name_exists(name)) {
        i++;
        name = makeMessage("ROI-", i);
    }
...
ROI* ROISet::new_region (const std::string& n) {
    current_region = NULL;
...
ROI* ROISet::set_current(const std::string& c) {
...
void ROISet::remove_region (const std::string& to_go) {
    for (int i=0; i<regions.size(); i++) {
        ROI* r = regions.at(i);
        if (r->get_name() == to_go) {
            regions.erase(regions.begin()+i, regions.begin()+i+1);
...
std::vector<ROI*> ROISet::get_regions () {
    return regions;
}
...
bool ROISet::name_exists (const std::string& name) {
    bool exists = false;
