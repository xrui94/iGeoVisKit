...
void ROIFile::saveSetToFile (ROISet* rs, bool append) {
    std::vector<ROI*> rlist = rs->get_regions();
    ROI* roi;
...
void ROIFile::saveRegionToFile (ROI* roi) {
    ...
    std::vector<ROIEntity*> elist;  // list of entities
    ROIEntity* re;             // current entity working on
    std::vector<coords> pts;        // list of points
    ...
    if (openFile.readSectionContent(section)=="") {
        ...
        if (openFile.readSectionContent("ROI List")!="") {
            ...
            openFile.updateBuffer("count", makeString(leader, (int)(totalRegions+1)));
    ...
    for (int i=0; i<totalEntities; i++) {
        ...
        openFile.updateBuffer(et, makeString(leader, re->type));
        ...
        for (int i=0; i<totalPoints; i++) {
            pt = pts.at(i);
            epc = makeString(ep, i);
            // 修复：将 coords 类型的 x 和 y 分别转换为字符串
            curPtXY = makeString(leader, (int)pt.x);
            curPtXY += ' ';
            curPtXY = makeString(curPtXY, (int)pt.y);
