    ...
    : QMainWindow(parent)
{
    ...
    m_overviewWindow = new OverviewWindow(this);
    overviewDock_->setWidget(m_overviewWindow);
...
void MainWindow::setupActions()
{
	...
	openAction_->setIcon(QIcon(":/icons/open.ico"));
	...
	zoomInAction_->setIcon(QIcon(":/icons/zoomin.svg"));
	zoomOutAction_ = new QAction("Zoom &Out (25%)", this);
	zoomOutAction_->setIcon(QIcon(":/icons/zoomout.svg"));
    ...
    connect(panAction_, &QAction::toggled, [this](bool checked) { if (imageWindow_) imageWindow_->setPanEnabled(checked); });
}
...
void MainWindow::setupToolBar()
{
	...
	// 工具栏仅保留用户需要的4个按钮：打开、放大、缩小、平移
	m_toolsBar->addAction(openAction_);
...
void MainWindow::openFile()
{
    ...
    QString initialDir;
    if (settingsFile) {
        std::string lastDir = settingsFile->getSetting("last", "open_dir", "");
        initialDir = QString::fromStdString(lastDir);
    }
    if (initialDir.isEmpty()) {
        initialDir = QDir::homePath();
    }

    // 选择文件后：加入左侧树并通过全局流程加载
    ...
    if (settingsFile) {
        ...
        settingsFile->setSetting("last", "open_dir", dir.toStdString());
    }
...
void MainWindow::loadFile(const QString& fileName) {
    ...
    if (!m_imageHandler || m_imageHandler->get_image_properties() == nullptr) {
        if (pw) pw->end();
        QMessageBox::critical(this, "iGeoVisKit Error", "Failed to open image file. Please check GDAL support and file path.");
        return;
    }
    ...
    setWindowTitle(QString("iGeoVisKit - ") + QFileInfo(fileName).fileName());

    // 接线：将 Renderer 与 OverviewGL 连接到概览窗口的 GLView
    if (m_overviewWindow && renderer_) {
        m_overviewWindow->attachRendererAndOverview(renderer_, m_imageHandler->getOverviewGL());
    }
    ...
    //m_imageHandler->redraw(); // 注意：可能这种统一通过事件通知的方式，可能会更好？？！！
    imageWindow_->getGLView()->update();
    m_overviewWindow->getGLView()->update();
    ...
    if (panAction_) panAction_->setEnabled(true);
}
...
void MainWindow::loadFile() {
    ...
    QString initialDir2;
    if (settingsFile) {
        std::string lastDir2 = settingsFile->getSetting("last", "open_dir", "");
        initialDir2 = QString::fromStdString(lastDir2);
    }
    if (initialDir2.isEmpty()) {
        initialDir2 = QDir::homePath();
    }
    ...
    // 保存本次选择的目录
    if (settingsFile) {
        QString dir2 = QFileInfo(fileName).absolutePath();
        settingsFile->setSetting("last", "open_dir", dir2.toStdString());
    }
    loadFile(fileName);
}
