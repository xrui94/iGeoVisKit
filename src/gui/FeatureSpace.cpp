...
{
	std::vector<ROI*> theROIs = regionsSet->get_regions();
	...
	if (!theROIs.empty()) //if there are some ROIs in the set
	{
		...
		for (int cr = 0; cr < theROIs.size(); cr++)
		{
			...
			std::vector<ROIEntity*> theEntities = currentROI->get_entities();
...
{
	std::vector<coords> theCoords = theEntity->get_points();
	int x = theCoords[0].x;
	int y = theCoords[0].y;
	int fx = x / LODfactor;
	int fy = y / LODfactor;
...
void FeatureSpace::getRectData(ROIEntity* theEntity, ROI* theROI)
{
	...
	for(int tiley = starty; tiley <= endy; tiley+=tileSize)
	{
		for(int tilex = startx; tilex <= endx; tilex+=tileSize)
		{
			...
			for(y = std::max(tiley, fy1); y < std::min((fy2 + 1), tiley + tileSize); y++)
			{
				for(x = std::max(tilex, fx1); x < std::min((fx2 + 1), tilex + tileSize); x++)
				{
...
{	
	...
	if (!polyPoints.empty())	//if we've got something to rasterise...
	{
		...
		boundsCoords.resize(vectorsize, new std::list<int>);
		...
		for(int tiley = starty; tiley <= endy; tiley+=tileSize)
		{
			...
			for(int tilex = startx; tilex <= endx; tilex+=tileSize)
			{
				...
				int loopstarty = std::max((tiley - yoffset), 0);
				int loopendy = std::min(vectorsize, tileendy - yoffset);
				assert(loopstarty >= 0);
				...
				for (int stepy = loopstarty; stepy < loopendy; stepy++)
				{
					...
					{
						std::list<int>::iterator j1 = boundsCoords[stepy]->begin();
						std::list<int>::iterator j2 = j1;
...
void FeatureSpace::isTurningPoint(int first, int middle, int last)
{
	   ...
	   (polyPoints[first].y < polyPoints[middle].y && polyPoints[last].y < polyPoints[middle].y)) //or if both neighbour y less than middle y
	{
		...
		{
			for(std::list<int>::iterator j = boundsCoords[polyPoints[middle].y - yoffset]->begin(); j != boundsCoords[polyPoints[middle].y - yoffset]->end(); j++)
			{
