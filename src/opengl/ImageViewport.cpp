...
ImageViewport::ImageViewport(ImageProperties* image_properties)
{
	...
	band_red = 1;
	band_green = std::min(2,num_bands);
	band_blue = std::min(3,num_bands);
}
...
//	zoom_value = round(zoom_value*100.0)/100.0;
    zoom_level = std::max(zoom_minimum, zoom_value);
    zoom_image_width = int(round(image_width * zoom_level));
    zoom_image_height = int(round(image_height * zoom_level));
	...
	image_x = std::min(image_width - viewport_width, image_x);
	image_y = std::min(image_height - viewport_height, image_y);
	
	image_x = std::max(image_x, 0);
	image_y = std::max(image_y, 0);
...
float ImageViewport::get_zoom_minimum(void)
{
	...
	zoom_minimum = std::min(horiz_min, vert_min);
	...
	zoom_minimum = std::min(zoom_minimum, (float)0.5);
...
{
	// 防御：窗口尺寸至少为 1，避免后续运算异常
	window_width = std::max(1, new_window_width);
	window_height = std::max(1, new_window_height);

	// 先计算最小缩放并确保当前缩放合法，避免 zoom_level==0 引发除零
	float min_zoom = get_zoom_minimum();
	if (zoom_level < min_zoom) {
		set_zoom_level(min_zoom);
		return; // set_zoom_level 已完成 viewport 尺寸与通知
	}
...
{
	band_red = std::min(band_R, num_bands);
	band_green = std::min(band_G, num_bands);
	band_blue = std::min(band_B,num_bands);
